...
#include </home/christina/CIntImage_to_Featurevec.h>

#define HEIGHT_ORIG 36//32
#define WIDTH_ORIG  36//44
...
public:
	...
	ros::Publisher vis_pub_ma;			//MarkerArray
        ros::Publisher gpTest;                     //sherly for testing
	...
	CCalc_Grasppoints(ros::NodeHandle nh)
	{
		...
		this->pubGraspPointsEval = nh.advertise<std_msgs::String>("/SVM/grasp_hypothesis_eval", 1);
                this->gpTest = nh.advertise<geometry_msgs::Pose>("jaco_poses_grasp",1); //sherly pub
		...
		this->box_center_x = -0.33;                //sherly //nnn
		this->box_center_y = -0.68;
		outputpath_full = "/home/christina/catkin_ws/src/calc_grasppoints_svm/tmp/features.txt";
		return_only_best_gp = false;
		...
		graspval_top = 119; //sherly how it came ???? maybe assuming something as the highest graspvalue //nnn
...
//********************************sherly not used anywhere so commented*****************************************************************//
...
void CCalc_Grasppoints::read_pc_cb(const sensor_msgs::PointCloud2ConstPtr& pc_in)
{
	...
	//set initial values for row,col,roll,tilt,topval for top grasp points
	id_row_top_overall = -1;
	id_col_top_overall = -1;
	nr_roll_top_overall = -1;
	nr_tilt_top_overall = -1;
...
void CCalc_Grasppoints::loop_control(pcl::PointCloud<pcl::PointXYZ> pcl_cloud_in)
{
   ...
   double timedif;
	...
	for (int tilt = 0; tilt < TILT_STEPS; tilt++)			// tilt loop
	{
		...
		{
			if (this->return_only_best_gp and (this->topval_gp_overall >= this->graspval_top)){//graspval_top somehow seemed to be assumed
			...
			//print_heights(roll,tilt); //sherly for now
...
void CCalc_Grasppoints::generate_grid(int roll, int tilt, pcl::PointCloud<pcl::PointXYZ> pcl_cloud_in)
{
	...
	for (size_t i = 0; i < pcl_cloud_transformed.points.size(); ++i)
	{
		...
		if ((pnt.x > this->box_center_x-r_row_m) and (pnt.x < this->box_center_x+r_row_m) and
			(pnt.y > this->box_center_y-r_col_m) and (pnt.y < this->box_center_y+r_col_m))//used to see if the point is inside the defined region
	...
	// david ausgeschalten!!!
//sherly commented this stuff from original code as not required 
//	if (tilt > 110){
//		for (int i = 0; i < nr_rows; i++){
//			for (int j = 0; j < nr_cols; j++){
...
void CCalc_Grasppoints::calc_featurevectors(int roll, int tilt)
{
	...
	for (int row = 0; row < HEIGHT - 14; row++){
		for (int col = 0; col < WIDTH -14; col++)   //sherly why -14????
		{
...
void CCalc_Grasppoints::pnt_in_box(int nr_roll, int nr_tilt){
	...
	bool print_pnt_in_box = false; //it was false
    //function:
    //   ( cx )               ( cos(alpha) )
    //g: (    ) +  lambda *   (            ) = r
    //   ( cy )               ( sin(alpha) )
    ...
    float boarder = 1.0;  // the border 7.0				//sherly		//ungraspable inside in box (because hand would touch box
    ...
    for (int i = 0; i < HEIGHT; i++){
        for (int j = 0; j < HEIGHT; j++){	//(2 times HEIGHT works because HEIGHT and WIDTH are equal!!)
            ...
            if ((i>1 and i < HEIGHT-1 and j>1 and j<HEIGHT-1)           and	   // take only points which are not in the 7cm boarder of the 56x56 grid (otherwise no featurecalc possible)
...
void CCalc_Grasppoints::predict_bestgp_withsvm(bool svm_with_probability){
	...
	try{
		...
		string pkg_path = string("/home/christina/catkin_ws3/src/calc_grasppoints_svm");// sherly change to a path where libsvm-3.12 is used
		//scale: use existing scaling file to scale feature values

		stringstream ss, ss2;

		//old:
		ss << pkg_path << "/libsvm-3.12/svm-scale -r " << pkg_path << "/data/range21062012_allfeatures /home/christina/catkin_ws3/src/calc_grasppoints_svm/tmp/features.txt > /home/christina/catkin_ws3/src/calc_grasppoints_svm/tmp/features.txt.scale";
		//new:
		...
		if (!svm_with_probability){
			...
			ss2 << pkg_path << "/libsvm-3.12/svm-predict /home/christina/catkin_ws3/src/calc_grasppoints_svm/tmp/features.txt.scale " << pkg_path << "/data/all_features.txt.scale.model /home/christina/catkin_ws3/src/calc_grasppoints_svm/tmp/output_calc_gp.txt";
			string command2 = ss2.str();
			i = system(command2.c_str());

		} else {
			//	trained with all manual examples, using probability as output
			i = system("/libsvm-3.12/svm-predict -b 1 /home/christina/catkin_ws3/src/calc_grasppoints_svm/tmp/features.txt.scale /libsvm-3.12/tools/allmanualfeatures.txt.scale.p.model /home/christina/catkin_ws3/src/calc_grasppoints_svm/tmp/output_calc_gp.txt"); //sherly this folder /usr/lib/libsvm not available 
		}
		cout << "\n\n i (predict): " << i << endl;
	} catch (int j) {
...
void CCalc_Grasppoints::show_predicted_gps(int nr_roll, int tilt, bool svm_with_probability){
	//PARAMETERS
	string path_svm_output =  	"/home/christina/catkin_ws3/src/calc_grasppoints_svm/tmp/output_calc_gp.txt";
	...
	if (printgraspseval) cout << " sherly graspseval[row][col]" << "\n\n";
...
void CCalc_Grasppoints::gp_to_marker(visualization_msgs::Marker *marker, float x, float y, float z, float green, bool pubmarker, int gripperwidth=1){
	...
	//visualization_msgs::Marker marker;
	(*marker).header.frame_id = "root";//"marker"; //"hobbit_wrt_down_right_cam" david
...
void CCalc_Grasppoints::transform_gp_in_wcs_and_publish(int id_row_top_all, int id_col_top_all,int nr_roll_top_all, int nr_tilt_top_all, int scaled_gp_eval){
	...
	//y_gp_roll += 0.01; sherly commented
	//cout << "moved gp by 1 cm in y direction (openrave)" << endl;  sherly commented

	//h_locmax_roll -= 0.02; sherly commented
...
gpPose.position.x=(gp1_wcs[0]+gp2_wcs[0])/2;
gpPose.position.y=(gp1_wcs[1]+gp2_wcs[1])/2;
gpPose.position.z=(gp1_wcs[2]+gp2_wcs[2])/2-0.1;
gpPose.orientation.x=0;
gpPose.orientation.y=0;
gpPose.orientation.z=0;
gpPose.orientation.w=1;

        gpTest.publish(gpPose); //sherly publish
